...
async function checkPermissions(): Promise<SupabaseClient<Database, "public">> {
  ...
  const userData = data as { role: "developer" | "manager" | "admin" } | null;

  if (userError || !userData || (userData.role !== 'manager' && userData.role !== 'admin')) {
    throw new Error("Forbidden: Insufficient permissions");
  }

  return supabase as unknown as SupabaseClient<Database, "public">;
}
...
export async function getAvailableUsers(cohortId: string) {
  try {
    ...
    const { data: currentMembers, error: membersError } = await (supabase as any)
      .from("cohort_members")
      .select("user_id")
      .eq("cohort_id", cohortId);
