...
jest.mock('@/app/actions/cohorts', () => ({
  ...
  getCohortMembers: jest.fn().mockResolvedValue([]),
  getAvailableUsers: jest.fn().mockResolvedValue([]),
}));
...
describe('TeamTable', () => {
  ...
  it('allows editing a cohort', async () => {
    ...
    // Verify UI updated - note: Modal closes on save, so we only check if updateCohort was called
    // In a real app, router.refresh() would update the list
  });

  it('allows creating a new cohort', async () => {
    render(<TeamTable members={[]} />);

    // Click Create Cohort
    fireEvent.click(screen.getByText('Create Cohort'));

    // Check if modal is open with empty fields
    ...
    fireEvent.change(screen.getByLabelText('Coaching Plan'), { target: { value: 'New Plan' } });

    // Click Save
    fireEvent.click(screen.getByText('Save'));

    // Verify createCohort called
    await waitFor(() => {
      expect(createCohort).toHaveBeenCalledWith({
        name: 'New Cohort',
        description: 'New Description',
        coaching_plan: 'New Plan',
        criteria: {}
      });
    });
  });

  it('allows deleting a cohort', async () => {
     // Mock window.confirm
    ...
    // Click Edit to see Delete button
    fireEvent.click(screen.getByText('Edit'));

    // Click Delete
    fireEvent.click(screen.getByText('Delete'));

    // Verify deleteCohort called
    await waitFor(() => {
      expect(deleteCohort).toHaveBeenCalledWith('c5');
    });
  });
});
