...
describe('cohortDetection', () => {
  ...
  it('should process cohorts correctly for matching users', async () => {
    ...
    // 4. check existing member (select single) - NEW: added this check in implementation
    queueResponse({ data: null, error: { code: 'PGRST116' } }); // Not found (so we can send email)
