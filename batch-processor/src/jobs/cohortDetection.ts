  ...
  for (const cohortDef of COHORTS) {
    ...
    for (const userMetrics of userMetricsMap.values()) {
        if (cohortDef.check(userMetrics)) {
            ...
            // But upsert handles the DB part. We want to know if it's NEW to send email.
            // A simple way is to check if the upsert actually inserted? Supabase upsert result doesn't explicitly say "inserted" vs "updated" easily without returning full row and checking created_at, but we set joined_at to now... which overwrites.
            // Better: Check existence first if we want to send email only on NEW entry.
            ...
            if (memberError) {
                ...
                // Send email if new member
                if (!existingMember) {
                    // Fetch user email (assuming user_id is email or we can look it up)
                    // Handoff says: "VS Code extension now uses the user's email ... as user_id"
                    // So userMetrics.user_id IS the email.
    ...
    await logAudit(
      "update_cohort_members",
      "cohorts",
      cohortId,
      {
        cohort_name: cohortDef.name,
        member_count: memberCount,
        description: `Updated cohort members based on criteria: ${JSON.stringify(cohortDef.criteria)}`
      }
    );
