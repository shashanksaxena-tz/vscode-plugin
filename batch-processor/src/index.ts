...
import { detectCriticalAlerts } from "./jobs/detectCriticalAlerts";
import { peerMentoring } from "./jobs/peerMentoring";
...
// Hourly: Detect critical alerts (e.g. high retry counts)
cron.schedule("0 * * * *", async () => {
  console.log("Running critical alerts detection...");
  await runJob("detectCriticalAlerts", detectCriticalAlerts);
});

// Weekly on Monday at 6 AM: Peer Mentoring matching
cron.schedule("0 6 * * 1", async () => {
  console.log("Running peer mentoring matching...");
  await runJob("peerMentoring", peerMentoring);
});
